...
private fun getSolutionA(filename: String) {
    ...
    val max = rows.size - 1
    val diag1Count = getDiag1(rows, max).sumOf { countXmas(it) }
    val diag2Count = getDiag2(rows, max).sumOf { countXmas(it) }
    val diag3Count = getDiag3(rows, max).sumOf { countXmas(it) }
    val diag4Count = getDiag4(rows, max).sumOf { countXmas(it) }
...
private fun getDiag1(lines: List<String>, max: Int): List<String> {
